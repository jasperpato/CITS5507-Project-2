      ...
      for(int pi = (int)(P_MIN*P_RES); pi <= (int)(P_MAX*P_RES); pi+=(int)(P_STEP*P_RES)) {
        ...
        for(int n_threads = NT_MIN; n_threads <= NT_MAX; n_threads+=NT_STEP) {
          ...
          if(pid == -1) exit(EXIT_FAILURE);
            ...
            args[5] = malloc(ARG_LENGTH*sizeof(char)); sprintf(args[5], "%d", n);
            args[6] = malloc(ARG_LENGTH*sizeof(char)); sprintf(args[6], "%f", p);
            args[7] = malloc(ARG_LENGTH*sizeof(char)); sprintf(args[7], "%d", n_threads);
